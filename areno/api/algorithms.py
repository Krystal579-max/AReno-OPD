...
from areno.api.loss_fns import dpo_loss_fn, grpo_loss_fn, gspo_loss_fn, opd_loss_fn, ppo_loss_fn, sft_loss_fn
...
def _load_ppo_trainer() -> type:
    ...
    return PPOTrainer
def _load_opd_trainer() -> type:
    from areno.api.trainers.opd import OPDTrainer
    return OPDTrainer
...
def _bind_opd_loss(config: TrainerConfig, loss_fn: Callable) -> Callable:
    from functools import partial
    return partial(loss_fn, kl_coef=getattr(config, "opd_kl_coef", 1.0), temperature=getattr(config, "opd_temperature", 1.0))
