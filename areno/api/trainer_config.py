...
`OPDTrainerConfig` adds the teacher (reference) checkpoint path and OPD-specific
hyperparameters for on-policy distillation.
"""
...
class OPDTrainerConfig(RolloutTrainerConfig):
    """On-Policy Distillation trainer configuration.
    ...
    checkpoint when ``ref_ckpt`` is not configured.
    ``opd_kl_coef`` scales the KL divergence loss, and ``opd_temperature``
    softens the log-probability distributions before computing the KL.
    """
    ref_ckpt: str | None = None
